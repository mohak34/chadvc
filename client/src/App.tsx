...
function App() {
  ...
  const { user, isAuthenticated, accessToken, logout, refreshAccessToken, validateSession, isValidating } = useAuthStore();
  const { setVoiceUsers, reset: resetVoice } = useVoiceStore();
  ...
  useEffect(() => {
    if (isInitializing || isValidating) return;
    ...
    const unsubscribeConnection = wsManager.onConnectionChange((connected) => {
      ...
      if (!connected) {
        ...
        setTimeout(() => clearInterval(checkReconnect), 60000);
      } else {
        setShowConnectionBanner(false);
        setReconnecting(false);
      }
    ...
    const unsubscribeVoiceKicked = webrtcManager.onVoiceKicked((reason) => {
      console.log("Kicked from voice:", reason);
      resetVoice();
      if (reason === "joined_from_another_device") {
        // Could show a toast here
        console.log("Voice disconnected: You joined from another device");
      }
    });
  ...
  // Show loading while initializing/validating
  if (isInitializing || isValidating) {
    return (
      <div className="min-h-screen bg-chad-bg-darkest flex items-center justify-center">
        <div className="text-chad-muted">Loading...</div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-screen bg-chad-bg-darkest text-chad-platinum">
      {/* Connection Banner */}
      {showConnectionBanner && (
          ...
          <div className="p-3 bg-chad-bg-darkest border-t border-chad-border">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className={`text-xs ${isConnected ? "text-green-500" : "text-red-400"}`}>
